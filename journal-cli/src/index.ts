...
/*
 * This file is the main driver for the Midnight journal example.
...
import { ledger, type Ledger, State } from '../../contract/src/managed/journal/contract/index.cjs';
import {
...
/* **********************************************************************
 * getJournalLedgerState: a helper that queries the current state of
 * the data on the ledger, for a specific journal contract.
 ...
 * in the journal contract.
 */

export const getJournalLedgerState = async (
...
You can do one of the following:
  1. Deploy a new journal contract
  2. Join an existing journal contract
  3. Exit
Which would you like to do? `;

const deployOrJoin = async (providers: JournalProviders, rli: Interface, logger: Logger): Promise<JournalAPI | null> => {
  let api: JournalAPI | null = null;

  while (true) {
    const choice = await rli.question(DEPLOY_OR_JOIN_QUESTION);
    switch (choice) {
      case '1':
        api = await JournalAPI.deploy(providers, logger);
        logger.info(`Deployed contract at address: ${api.deployedContractAddress}`);
        return api;
      case '2':
        api = await JournalAPI.join(providers, await rli.question('What is the contract address (in hex)? '), logger);
 ...
 * by the contract to be in the ledger state of the journal.
 */

const displayLedgerState = async (
  providers: JournalProviders,
  deployedJournalContract: DeployedJournalContract,
  logger: Logger,
): Promise<void> => {
  const contractAddress = deployedJournalContract.deployTxData.public.contractAddress;
  const ledgerState = await getJournalLedgerState(providers, contractAddress);
  if (ledgerState === null) {
    logger.info(`There is no journal contract deployed at ${contractAddress}`);
  } else {
    const entryState = ledgerState.state === State.OCCUPIED ? 'occupied' : 'vacant';
    const latestEntry = !ledgerState.entry.is_some
      ? 'none'
      : `${ledgerState.entry.value.thought} (rating: ${ledgerState.entry.value.rating})`;
    logger.info(`Current state is: '${entryState}'`);
    logger.info(`Current entry is: '${latestEntry}'`);
...
const displayPrivateState = async (providers: JournalProviders, logger: Logger): Promise<void> => {
  const privateState = await providers.privateStateProvider.get(journalPrivateStateKey);
  if (privateState === null) {
    logger.info(`There is no existing journal private state`);
  } else {
 ...
 * determine if the current user is the owner of the current entry.
 */

const displayDerivedState = (ledgerState: JournalDerivedState | undefined, logger: Logger) => {
  if (ledgerState === undefined) {
    logger.info(`No journal state currently available`);
  } else {
    const entryState = ledgerState.state === State.OCCUPIED ? 'occupied' : 'vacant';
    const latestEntry = ledgerState.entry
      ? (typeof ledgerState.entry === 'string' 
         ? ledgerState.entry 
         : (typeof ledgerState.entry === 'object' && ledgerState.entry !== null && 'thought' in ledgerState.entry && 'rating' in ledgerState.entry)
           ? `${(ledgerState.entry as { thought: string; rating: number }).thought} (rating: ${(ledgerState.entry as { thought: string; rating: number }).rating})`
           : JSON.stringify(ledgerState.entry))
      : 'none';
    logger.info(`Current state is: '${entryState}'`);
    logger.info(`Current entry is: '${latestEntry}'`);
    logger.info(`Current sequence is: ${ledgerState.sequence}`);
...
/* **********************************************************************
 * mainLoop: the main interactive menu of the journal CLI.
...
const mainLoop = async (providers: JournalProviders, rli: Interface, logger: Logger): Promise<void> => {
  const journalApi = await deployOrJoin(providers, rli, logger);
  if (journalApi === null) {
    return;
  }
  let currentState: JournalDerivedState | undefined;
  const stateObserver = {
    next: (state: JournalDerivedState) => (currentState = state),
  };
  const subscription = journalApi.state$.subscribe(stateObserver);
  try {
    while (true) {
      const choice = await rli.question(MAIN_LOOP_QUESTION);
      switch (choice) {
        case '1': {
          const thought = await rli.question(`What thought do you want to record? `);
          const ratingStr = await rli.question(`What rating (1-10) do you want to assign? `);
          const rating = parseInt(ratingStr, 10);
          await journalApi.post(thought, rating);
          break;
        }
        case '2':
          ...
          break;
        case '3':
          await displayLedgerState(providers, journalApi.deployedContract, logger);
          break;
        case '4':
...
/* **********************************************************************
 * The wallet + provider code remains unchanged from the bboard example.
 * It is reused here to support transactions and contract interaction.
*/
...
/* **********************************************************************
 * run: the main entry point that starts the whole journal CLI.
 *
...
export const run = async (config: Config, logger: Logger, dockerEnv?: DockerComposeEnvironment): Promise<void> => {
  ...
  if (dockerEnv !== undefined) {
    ...
    if (config instanceof StandaloneConfig) {
      config.indexer = mapContainerPort(env, config.indexer, 'journal-indexer');
      config.indexerWS = mapContainerPort(env, config.indexerWS, 'journal-indexer');
      config.node = mapContainerPort(env, config.node, 'journal-node');
      config.proofServer = mapContainerPort(env, config.proofServer, 'journal-proof-server');
    }
  }
  ...
  try {
    if (wallet !== null) {
      ...
      const providers = {
        ...
        zkConfigProvider: new NodeZkConfigProvider<'post' | 'delete'>(config.zkConfigPath),
